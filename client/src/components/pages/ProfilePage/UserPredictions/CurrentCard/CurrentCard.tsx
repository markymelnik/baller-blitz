...
export const CurrentCard = ({ gameInfo }: CurrentCard) => {
	...
	return (
		<li className="current-card">
			<div className="current-date">{GameDataFormatter.formatDate(game_date)}</div>
			<div className="current-matchup">{away_team} vs {home_team}</div>
			<div className="current-predicted-winner">Predicted: {predicted_winner}</div>
		</li>
	)
}